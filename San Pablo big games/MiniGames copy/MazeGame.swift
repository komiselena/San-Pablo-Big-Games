...
class MazeGameScene: SKScene, ObservableObject {
    ...
    private let playerRadius: CGFloat = 2
    let moveStep: CGFloat = 3
    private let startPoint = CGPoint(x: 10, y: 187)
    ...
    override func didMove(to view: SKView) {
        ...
        mazeBackground.fillColor = SKColor(hex: "EBB36A")
        ...
        flagNode = SKSpriteNode(imageNamed: "cube")
    ...
    private func isWall(at point: CGPoint) -> Bool {
        ...
        // Цвет фона #EBB36A в RGB: (235, 179, 106)
        let isBackground = (r >= 0.92 && r <= 0.93) &&  // 235/255 ≈ 0.9215
                          (g >= 0.70 && g <= 0.71) &&  // 179/255 ≈ 0.7019
                          (b >= 0.41 && b <= 0.42)     // 106/255 ≈ 0.4156
        
        let isPlayer = (r == 1.0 && g == 0.0 && b == 0.0) // Красный цвет игрока
        let white = (r == 0.0 && g == 0.0 && b == 0.0)
        let some = (r == 0.0 && g == 0.0 && b == 0.4392156862745098)
        // Если это не фон и не игрок, значит это стена
        print("Color at \(point): R:\(r) G:\(g) B:\(b)")
        return !isBackground && !isPlayer && !white && !some
    }
